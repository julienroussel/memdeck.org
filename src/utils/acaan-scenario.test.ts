...
describe("calculateCutDepth", () => {
  it("returns 0 when card is already at target position", () => {
    expect(
      calculateCutDepth(createDeckPosition(1), createDeckPosition(1))
    ).toBe(0);
    expect(
      calculateCutDepth(createDeckPosition(26), createDeckPosition(26))
    ).toBe(0);
    expect(
      calculateCutDepth(createDeckPosition(52), createDeckPosition(52))
    ).toBe(0);
  });
  ...
  it("calculates correct cut depth when card is ahead of target (no wraparound)", () => {
    ...
    expect(
      calculateCutDepth(createDeckPosition(10), createDeckPosition(5))
    ).toBe(5);
    // Card at position 52, target is position 1 -> cut 51 cards
    expect(
      calculateCutDepth(createDeckPosition(52), createDeckPosition(1))
    ).toBe(51);
    // Card at position 30, target is position 20 -> cut 10 cards
    expect(
      calculateCutDepth(createDeckPosition(30), createDeckPosition(20))
    ).toBe(10);
  });
  ...
  it("calculates correct cut depth with wraparound (target ahead of card)", () => {
    ...
    expect(
      calculateCutDepth(createDeckPosition(5), createDeckPosition(10))
    ).toBe(47);
    // Card at position 1, target is position 52 -> cut 1 card
    // Formula: (1 - 52 + 52) % 52 = 1
    expect(
      calculateCutDepth(createDeckPosition(1), createDeckPosition(52))
    ).toBe(1);
    // Card at position 20, target is position 30 -> cut 42 cards
    // Formula: (20 - 30 + 52) % 52 = 42
    expect(
      calculateCutDepth(createDeckPosition(20), createDeckPosition(30))
    ).toBe(42);
  });
  ...
  it("handles edge cases at deck boundaries", () => {
    ...
    expect(
      calculateCutDepth(createDeckPosition(1), createDeckPosition(2))
    ).toBe(51);
    // Card at position 2, target is position 1 -> cut 1 card
    expect(
      calculateCutDepth(createDeckPosition(2), createDeckPosition(1))
    ).toBe(1);
    // Card at position 51, target is position 52 -> wraparound, cut 51 cards
    expect(
      calculateCutDepth(createDeckPosition(51), createDeckPosition(52))
    ).toBe(51);
    // Card at position 52, target is position 51 -> cut 1 card
    expect(
      calculateCutDepth(createDeckPosition(52), createDeckPosition(51))
    ).toBe(1);
  });
