...
- **Guided Discovery** (`src/hooks/use-feature-discovery.ts`, `src/components/next-challenge-card.tsx`): Nudges users toward untried modes and sub-variants, deduced from their own session history (`deriveFeatureUsage`). Surfaces one dismissible "New challenge" card at a time on the home page — earned after ≥3 sessions, personalized to the most-used mode, retire-on-accept, retire-and-snooze-the-surface-on-dismiss, and silent while the share nudge is pending. Engine lives in `src/utils/{feature-usage,suggestion-catalog,discovery-typeguards}.ts`; the catalog covers untried whole modes and sub-variants (timed suggestions and the post-session/Stats surfaces are tracked in epic #693). **Deep-link preselect**: the "Try it" CTA links to `${ROUTES.x}?try=<value>` (and `?timed=1`); the target mode page consumes it on mount via `useSuggestionDeepLink` (`src/hooks/use-suggestion-deep-link.ts`), validating the value against its own enum guard before applying it through the existing setter, then stripping the param with a replace navigation. Deep-link URLs **must keep the route's trailing slash** (`/flashcard/?try=…`) — a slashless form hits the SEO 301 redirect that can drop the query string
